...
| `loop` | Generic native-looping primitive. Loops an arbitrary prompt (literal `--prompt` or `--prompt-file <path>`) until the completion promise is emitted or `--max-iterations` is reached. Per-session state at `.synthex/loops/<loop-id>.json`; supports `--resume <id>` / `--resume-last`. Coexists with `ralph-loop` plugin (native takes precedence). | — |
...
| `next-priority` | Execute next highest-priority tasks. Supports `--loop` for native iteration until every task is `done` (or milestone-boundary exit). | Tech Lead |
