...
If `<project_root>/.synthex-plus/` does not exist, print:

```
No .synthex-plus/ directory in this project. Run /synthex-plus:team-init first, or visit https://github.com/bluminal/lumenai to star the repo directly.
```
...
3. Update `.synthex-plus/state.json` to mark `starred: true`. Use the **Write** tool, preserving any existing `last_seen_version` and `dismissed` fields. The full state document after update is:

   ```json
   {
     "schema_version": 1,
     "last_seen_version": "<preserved from existing state, or current synthex-plus plugin version if absent>",
...
2. Update `.synthex-plus/state.json` to mark `star_dismissed: true`. Use the **Write** tool, preserving any existing fields. Full document:

   ```json
   {
     "schema_version": 1,
     "last_seen_version": "<preserved, or current synthex-plus plugin version if absent>",
     "dismissed": <preserved, default false>,
...
The `.synthex-plus/state.json` file is per-developer/per-clone (gitignored). This command extends it with two optional fields:
