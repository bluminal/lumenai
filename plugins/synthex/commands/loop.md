...
- **`--max-iterations` > 200 OR < 1 OR non-integer** — refuse: `--max-iterations must be an integer in [1, 200]; got <value>.` (FR-NL42)
...
- **`--name <slug>` supplied (fresh start)**: use the slug verbatim. If `.synthex/loops/<slug>.json` exists with `status == "running"`, refuse: `Loop "<slug>" is already running (iteration <N>/<M>). Use /synthex:loop --resume <slug> to continue or /synthex:cancel-loop <slug> to stop it.` If the existing file is in a terminal status, archive it (next step) and proceed.
...
7. **Idle wait (only when the iteration had nothing to do).** If the prompt's work was a no-op this iteration — e.g. it is waiting on an external change — print one line, then run `plugins/synthex/scripts/loop-idle-wait.sh <loop-id> [watch-path …]` (resolved from the installed plugin root; in Claude Code, `bash "${CLAUDE_PLUGIN_ROOT}/scripts/loop-idle-wait.sh" …`) as ONE foreground shell command with the tool's timeout set to at least 600 seconds (Claude Code: Bash `timeout: 600000`). On other hosts (Codex, Gemini CLI, OpenCode, Grok, Hermes), or if `${CLAUDE_PLUGIN_ROOT}` is empty, use the installed plugin root: `plugin_root` from `.synthex/state.json`, else the directory two levels above the wrapper you were loaded from. If the host caps shell timeouts lower, prefix `SYNTHEX_LOOP_IDLE_MAX=<cap minus 30>`. Pass the files whose change would make the next iteration productive (the `--prompt-file`, a plan, a status file). It returns on change, on the loop leaving `running`, or after a backoff limit (60s → 120s → 300s → 540s). See [Idle iterations](../hooks/loop-advance-gate.md#idle-iterations). If the gate's block reason (Claude Code only) gives an absolute path, prefer that; if the script cannot be found, skip the wait and continue — never end the turn instead.
8. **Continue to the next iteration — in the SAME assistant turn.** Re-enter step 1 without ending your turn. If you do end your turn while the loop is still `running` and unfinished, the [`loop-advance-gate`](../hooks/loop-advance-gate.md) Stop hook re-invokes you for the next iteration — a turn-end is recovered, not fatal (ADR-003) — but every turn-end prints a "Stop hook error" line and triggers "agent finished" notifications from other tools' Stop hooks (e.g. Orca), so treat the gate as a safety net, not the driver. On hosts without the Stop hook (Codex, Grok, and others) there is no gate at all: a turn-end stops the loop. The one self-inflicted way to break the loop early is emitting the completion promise before the completion conditions hold; emit it only when you truly intend to terminate. To stop intentionally, emit the promise or run `/synthex:cancel-loop <loop-id>`.
