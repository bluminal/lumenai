/**
 * Typed assertion helpers for the artifact-as-largest-file context-bundle fixture.
 *
 ...
 *
 * Scenario: artifact (80 KB) exceeds max_file_bytes (50000) but is below max_bundle_bytes.
 * Assembler MUST emit narrow_scope_required rather than summarizing the artifact.
 * Exercises Behavioral Rule 1 on the per-file-cap breach path.
 */
...
export interface ArtifactAsLargestFileFixture {
  scenario: string;
  description: string;
  input: {
...
/**
 * Validates the top-level structure of the artifact-as-largest-file fixture.
 * Returns a ValidationResult.
 */
export function assertArtifactAsLargestFileFixtureShape(fixture: unknown): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function assertStatusIsError(fixture: ArtifactAsLargestFileFixture): string | null {
 ...
 * Returns null on pass, error string on fail.
 */
export function assertErrorCodeIsNarrowScope(fixture: ArtifactAsLargestFileFixture): string | null {
 ...
 * Returns null on pass, error string on fail.
 */
export function assertArtifactNotSummarized(fixture: ArtifactAsLargestFileFixture): string | null {
 ...
 * Returns null on pass, error string on fail.
 */
export function assertBehavioralRule1Enforced(fixture: ArtifactAsLargestFileFixture): string | null {
...
/**
 * Asserts that the artifact size exceeds max_file_bytes (this fixture's trigger condition).
 * Returns null on pass, error string on fail.
 */
export function assertArtifactExceedsPerFileCap(fixture: ArtifactAsLargestFileFixture): string | null {
  const { artifact_size_bytes, max_file_bytes } = fixture.expected;
  if (artifact_size_bytes <= max_file_bytes) {
    return (
      `expected.artifact_size_bytes (${artifact_size_bytes}) should exceed ` +
      `expected.max_file_bytes (${max_file_bytes}) to trigger the per-file-cap error path`
    );
  }
  return null;
}
 ...
 * Returns a ValidationResult.
 */
export function assertErrorOutputShape(fixture: ArtifactAsLargestFileFixture): ValidationResult {
  const errors: string[] = [];
