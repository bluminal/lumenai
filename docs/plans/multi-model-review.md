...
|---|----------|--------|--------|
| Q1 | (OQ-2 follow-up) If `llm embed` is unavailable, should v1's Stage 3 milestone require `llm embed` or fall back to host-session embedding? | Affects whether the embedding milestone has an install precondition. Recommend: decide before starting the embedding milestone. | Resolved → D23 |
...
**Task 39 Completion Note:** Done. `plugins/synthex/agents/audit-artifact-writer.md` Haiku-backed utility agent. Command-agnostic per D20 (parameterized by `command`; reused unchanged across review-code + write-implementation-plan). All 7 FR-MR24 sections in order; Section 7 conditional. Skip-write when `audit.enabled: false`. Filename pattern `<YYYY-MM-DD>-<command>-<short-hash>.md`. NFR-MR4 usage verbatim with `usage: not_reported` fallback. Atomic .tmp + rename. 36 structural tests in `audit-writer-md.test.ts`. 6 `[T]` criteria pass; `[H]` (sample audit human-readability) approved during execution. Commit `4e421b7`.

**Task 40 Completion Note:** Done. `tests/schemas/audit-artifact.ts` validator + `audit-artifact.test.ts` (26 tests). Validates filename regex (both review-code + write-implementation-plan), 6 mandatory sections + conditional Continuation Event, native/external split, NFR-MR4 usage block (input_tokens+output_tokens) OR explicit `usage: not_reported`, raised_by attribution. All 4 `[T]` criteria pass. Commit `30b6043`.
...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 42 | Update `plugins/synthex/commands/write-implementation-plan.md` plan-review step: when multi-model is active, invoke `multi-model-review-orchestrator` with native sub-agents (Architect + `design-system-agent` + Tech Lead) AND external adapters in single parallel batch. PM agent receives consolidated findings in unchanged shape. **Note:** Task can begin after Phase 3; does not strictly require Phase 4. Coordinate with Phase 4 owners on Task 39 (Milestone 4.0) ownership when Phases 4 and 5 ship simultaneously. **Status:** Done. Step 6a in `write-implementation-plan.md` rewritten with two-branch structure. Multi-model path invokes `multi-model-review-orchestrator` with `command: 'write-implementation-plan'`, native_reviewers `[architect, design-system-agent, tech-lead]`, resolved config, per_reviewer_timeout_seconds. Native-only path byte-identical (FR-MR23). PM receives consolidated findings (Step 6c); plan-scribe applies edits unchanged (Step 6d). Bundled commit `d4e6e98`. | L | Phase 3 (orchestrator), Milestone 4.0 (audit writer) | done |
| 43 | Add `--multi-model` / `--no-multi-model` invocation flags per FR-MR6. **Status:** Done. Invocation Flags sub-section added per FR-MR6. --multi-model + --no-multi-model mutually exclusive; override master + per-command config; no complexity gate consulted (FR-MR22 contrast). Bundled commit `d4e6e98`. | S | Task 42 | done |
| 44 | Document that `plan-linter` (pre-review structural check) is unaffected — runs before orchestrator as today. **Status:** Done. Step 5.5 documents plan-linter is UNAFFECTED by multi-model and runs BEFORE the orchestrator (sole job: structural validation; not part of reviewer set). `[H]` criterion (plan-linter runs before orchestrator) approved during execution. Bundled commit `d4e6e98`. | S | Task 42 | done |
| 45 | Layer 2 fixture at `tests/fixtures/multi-model-review/write-implementation-plan/`: (a) draft plan + multi-model enabled → orchestrator invoked, native + external in one batch, PM receives consolidated findings; (b) multi-model disabled → byte-identical to the **redacted** Task 0 baseline (FR-MR23 regression). **Status:** Done. 2 fixtures under `tests/fixtures/multi-model-review/write-implementation-plan/`: multi-model-enabled (3 native + 2 external; consolidated findings with mixed attribution; audit file written) and multi-model-disabled (byte-identical to redacted Task 0 baseline; baseline snapshot verified to exist with redacted placeholders). Both `[T]` criteria pass. Bundled commit `236f29d`. | L | Tasks 42–44, Task 0 | done |
| 46 | Audit-artifact writer (Task 39, command-agnostic) reused; add Layer 2 assertion that audit file is written for write-implementation-plan invocations with the FR-MR24 file-naming pattern. **Status:** Done. Audit-artifact reuse verified via `audit-file-sample.md` in multi-model-enabled fixture. Filename pattern `<YYYY-MM-DD>-write-implementation-plan-<hash>.md` validates (D20 command-agnostic). Native reviewers section lists architect, design-system-agent, tech-lead — NOT code-reviewer or security-reviewer. Both `[T]` criteria pass. 38 tests in `write-implementation-plan-fixtures.test.ts`. Bundled commit `236f29d`. | S | Task 45, Milestone 4.0 | done |
...
**Task 47 Completion Note:** Done. Section 4 "Configure Multi-Model Review (optional)" added to `init.md` between concurrent-tasks (3) and .gitignore (renumbered to 5). Sub-sections 4a (parallel Bash detection scan, D22), 4b (3 AskUserQuestion options with detected-but-unauthenticated remediation hints), 4c (FR-MR27 data-transmission warning), 4d (preflight in FR-MR20 format + docs/reviews/ creation). Anti-pattern: no API keys in config (Synthex CLI-only). 36 tests in `init-multimodel-md.test.ts`. All 10 `[T]` criteria pass; all 3 `[H]` (3 options + progress indicator + warning style) approved during execution. Commit `b55d38b`.
...
**Task 48 Completion Note:** Done. 3 fixtures under `tests/fixtures/multi-model-review/init/`: enabled-with-detected (mixed auth scan; reviewers list = authenticated only; gemini surfaced separately with remediation hint), enabled-later-with-snippet (no config writes; YAML snippet validates with all 3 detected reviewers commented out), skip (no multi_model_review section). 53 tests in `init-fixtures.test.ts`. All 4 `[T]` criteria pass. Commit `d6f4f8f`.
...
**Task 49 Completion Note:** Done. `docs/specs/multi-model-review/architecture.md` skeleton REPLACED with FINAL (`## Status: Final`). 11 sections covering proposer+aggregator design, native vs external, fan-out (ASCII), D17 tier table, context bundle, 6-stage consolidation pipeline (5 in v1, Stage 3 deferred to Phase 7), failure handling (FR-MR16/17, NFR-MR2), audit artifact (D20), v1 vs v2 explicit table. 42 tests in `architecture-final-md.test.ts` + 84 tests (`architecture-md.test.ts` updated for skeleton→final). All cross-references verified: FR-MR9/11/12/13/14/14a/14b/15/16/17/20/23/24/28; NFR-MR2/4; D5/6/17/18/20/21. Both `[T]` pass; both `[H]` (architectural concerns coverage; v1 vs v2 explicit) approved during execution. Commit `db36b37`.
