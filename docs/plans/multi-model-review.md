...
| 22 | Author Layer 1 validator at `tests/schemas/orchestrator-output.ts` validating the orchestrator's unified output: per-reviewer status table (native + external clearly separated), aggregated findings list, path-and-reason header. Vitest suite. | M | Task 19 | in progress |
