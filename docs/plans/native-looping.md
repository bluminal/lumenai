...
| # | Task | Complexity | Dependencies | Req | Status |
|---|------|-----------|--------------|-----|--------|
| 34 | Author `tests/fixtures/native-looping/state-file-lifecycle/`. Sub-fixtures: `create`, `increment`, `complete`, `cancel`, `max-iter`, `crash-recover`. Each fixture has a pre-state directory layout, the state-file mutation applied by the iteration logic, and the expected post-state. Use vitest + temp dirs (matches `upgrade-nudge-hook-behavioral.test.ts` pattern from upgrade-onboarding Phase 5.2). **Resolution:** Authored `tests/helpers/loop-state-lifecycle.ts` (TS reference implementation mirroring the spec exactly) + `tests/schemas/loop-state-lifecycle.test.ts` (20 tests covering all 6 sub-fixtures via dynamic temp dirs). Since looping is pure markdown instructions (no script), the TS helper IS the testable surface — locks in the contract any agent must satisfy. | L | Task 4 | FR-NL14, FR-NL21–FR-NL23 | done |
| 35 | Author `tests/fixtures/native-looping/concurrent-sessions/`. Two pre-state state files for two different loop-ids; assert that mutating one does not affect the other. Validates NFR-NL2. **Resolution:** included in `loop-state-lifecycle.test.ts` under the "Concurrent sessions" describe block. | M | Task 34 | NFR-NL2, E2 | done |
| 36 | Author `tests/fixtures/native-looping/resume-flow/`. Three sub-fixtures: `resume-by-id`, `resume-last`, `resume-rejects-terminal-status`. **Resolution:** included in `loop-state-lifecycle.test.ts` under the "Resume flow" describe block (4 tests covering FR-NL26, FR-NL27, FR-NL40, terminal-status rejection). | M | Task 4 | FR-NL26, FR-NL27, FR-NL40 | done |
| 37 | Author timing fixture: invoke the iteration-boundary primitives 30 times; assert p95 wall-clock for state-file read + increment + write is ≤ 75 ms (loose for CI overhead; NFR-NL1 target is 200 ms full iteration boundary which includes the marker print). **Resolution:** included in `loop-state-lifecycle.test.ts` "Timing budget" describe block. | S | Task 34 | NFR-NL1 | done |
| 38 | Author promptfoo behavioral assertions for `/synthex:loop` covering: refusal on missing `--completion-promise`, refusal on both `--prompt` and `--prompt-file`, refusal on missing prompt file, refusal on `--max-iterations > 200`, refusal on `--resume` for unknown loop-id. Reuse the existing promptfoo provider. **Resolution:** Added NL-B1..NL-B5 entries to `tests/promptfoo.config.yaml`. Cache population deferred to first manual `npx promptfoo eval --filter-pattern "NL-B"` (matches UO-B convention). | M | Task 4 | FR-NL37–FR-NL42 | done |
| 39 | Author promptfoo behavioral assertion for the Ralph-precedence advisory line (FR-NL44): when `--loop` is set and `.claude/ralph-loop.local.md` is `active: true`, the command prints the advisory and does NOT mutate the Ralph state file. **Resolution:** NL-B6 entry added; targets `/synthex:next-priority --loop` with a pre-existing Ralph state file. | M | Phase 4 | FR-NL44, FR-NL45 | done |
...
| # | Task | Complexity | Dependencies | Req | Status |
|---|------|-----------|--------------|-----|--------|
| 40 | Update `plugins/synthex/README.md` — add a "Native Looping" section that summarizes the surface (one paragraph + a one-liner example for `/synthex:loop` and one for `next-priority --loop`). Cross-references `plugins/synthex/docs/native-looping.md` for the full spec. Also extends the Commands table to register `/loop`, `/list-loops`, `/cancel-loop` and bumps the count to 16. | S | All Phase 4 tasks | Docs | done |
| 41 | Update `plugins/synthex-plus/README.md` — add a "Native Looping" section noting the same surface is available on team commands; cross-reference the synthex doc. Documents lead-output-only promise scan (E7) and team-lifecycle independence (FR-NL35). | S | All Phase 5 tasks | Docs | done |
| 42 | Update `README.md` at the repo root — add a one-line bullet under the latest synthex highlights. **Resolution:** added a full "Native Looping" subsection (between Multi-Model Review and Automated Testing) summarizing the surface and cross-referencing the framework spec. | S | All Phase 4–5 tasks | Docs | done |
...
| 45 | Update `CLAUDE.md` if it contains a list of synthex commands — add `/synthex:loop`, `/synthex:list-loops`, `/synthex:cancel-loop`. **Resolution:** added 3 new rows to the Commands table (between `dismiss-upgrade-nudge` and `next-priority`) and amended `next-priority`'s description to mention `--loop` support. | S | Task 4, 8, 10 | Docs | done |
