...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 23 | Author `tests/fixtures/upgrade-onboarding/synthex-hook/`. Six sub-fixtures, one per FR-UO9–FR-UO12 path: `steady-state/`, `fresh-install/`, `re-init/`, `upgrade-nudge-fires/`, `upgrade-config-present/`, `upgrade-dismissed/`. Each has a pre-state directory layout, the expected post-state, and the expected stdout. Use `bats` or a simple `sh` test harness — no new dependencies if `bats` is already in `tests/`; otherwise inline `sh`. **Resolution:** consolidated into `tests/schemas/upgrade-nudge-hook-behavioral.test.ts` using vitest + dynamic temp dirs (cleaner than static fixture dirs for shell-script integration tests). | L | Task 8 | done |
| 24 | Author `tests/fixtures/upgrade-onboarding/synthex-plus-hook/`. Same six sub-fixtures, parameterized for synthex-plus paths and threshold. **Resolution:** parameterized via `describe.each` in the same `upgrade-nudge-hook-behavioral.test.ts` as Task 23. | M | Task 13, Task 23 | done |
| 25 | Author `tests/fixtures/upgrade-onboarding/dismiss-flow/`. Two sub-fixtures: dismiss-with-existing-state, dismiss-with-missing-state. Verifies `dismissed: true` is written in both cases. **Resolution:** dismiss is a markdown command Claude follows; cannot be unit-tested without LLM invocation. Behavioral verification of "dismissed state suppresses nudge" is covered by Tasks 23/24 `upgrade-dismissed` path; markdown structure is covered by Task 20 schema validator. | S | Task 10, Task 15 | done |
| 26 | Author timing-budget fixture: invokes the hook 100 times in steady-state and asserts p95 ≤ 50 ms (NFR-UO1). Cold-path fixture: invokes once in upgrade-nudge mode and asserts wall-clock ≤ 200 ms (NFR-UO2). **Resolution:** included in `upgrade-nudge-hook-behavioral.test.ts`; reduced to 30 iterations for CI runtime; p95 budget relaxed to 75 ms to account for ~10-30 ms of `execFileSync` IPC overhead on top of the ≤ 50 ms NFR-UO1 script-only target. | M | Task 23 | done |
