...
| D27 / ADR-003 | **External CLI permission model = Pattern 1 (read-only) by default, Pattern 3 (parent-mediated) auto-selected for CLIs that support it (Codex app-server, Claude Code canUseTool), Pattern 2 (sandbox-yolo) as per-CLI config opt-in.** Pattern 1 (read-only) by default: all adapters invoke CLIs with read-only/plan flags. Pattern 3 (parent-mediated) auto: Codex (`app-server` JSON-RPC `requestApproval`) and Claude Code (`canUseTool` over `--output-format stream-json`) use native approval-proxying — no config needed. Pattern 2 (sandbox-yolo): opt-in per CLI via `multi_model_review.external_permission_mode.<cli-name>: sandbox-yolo` with required user confirmation before spawn. **See "ADR-003 Known Limitations" subsection immediately below for the four threat-model gaps acknowledged but out of scope for v1.** | FR-MMT21; security; user request (2026-04-28) | Pattern 1 (read-only) is safe default for all environments. Pattern 3 (parent-mediated) is chosen for CLIs with native support to give smooth UX without blocking. Pattern 2 (sandbox-yolo) is an explicit opt-in so users can't accidentally grant full permissions. |

### ADR-003 Known Limitations (out of scope for v1)

The three-pattern external CLI permission model (D27 / ADR-003) ships safe-by-default and addresses the primary attack surfaces of running third-party CLIs as review proposers. The following four threat-model gaps were surfaced by the 2026-04-29 multi-model team-review of Phase 11.1 and are **acknowledged but explicitly out of scope for v1**. Each is tracked here so consumers understand what v1 protects against and what it does not, and so future hardening work has a clear starting list.

1. **Supply-chain — no binary integrity check.** ADR-003 trusts the documented flag behavior of the `codex`, `gemini`, `claude`, and other external CLI binaries. There is no signature verification, checksum pinning, or known-good version enforcement on the binaries themselves. A compromised or replaced binary in `PATH` could ignore `--readonly` / `--approval-mode never`, lie about `app-server --help` exit codes (silently degrading Pattern 3 → Pattern 1), or exfiltrate context bundle contents to an attacker-controlled endpoint. **Mitigation deferred:** post-v1 — add per-CLI version pinning + checksum verification at preflight time.

2. **File-read scope on agentic adapters.** Pattern 1 (read-only) prevents writes and tool-use side-effects, but does NOT restrict what files an agentic adapter (Codex, Gemini) can READ within its sandbox. The CLI may read `.env`, `~/.aws/credentials`, SSH private keys, or any other file the OS sandbox permits — and may surface that content into its findings JSON, audit artifacts, or external API requests. There is no documented allow-list / deny-list for read-set scope. **Mitigation deferred:** post-v1 — add a `read_scope` config (default-allow with explicit deny patterns for known-secret paths) enforced via OS sandbox profile and/or context-bundle assembler exclusion rules.

3. **CI bypass of sandbox-yolo confirmation.** Task 89's non-TTY guard (Phase 11.2) addresses the unbounded-CI-hang case by aborting cleanly when stdin is not a TTY. It does **not** prevent a deliberate bypass: a CI script that pipes `y\n` to stdin will satisfy the prompt and proceed in sandbox-yolo mode. ADR-003 does not currently prohibit `sandbox-yolo` configuration in CI contexts or require a separate environment-level opt-in for non-interactive use. **Mitigation deferred:** post-v1 — add an `allow_sandbox_yolo_in_ci: false` (default) config flag that fails sandbox-yolo invocations when `CI=true` is set in the environment, requiring an explicit project-level override.

4. **Rate limiting — no per-session/per-hour cap on external API invocations.** Standing pools with short TTLs and automated submitters can trigger continuous fan-out to paid external APIs (OpenAI Codex, Google Gemini) without any documented cap. A misconfigured submitter or an adversarial action could rack up significant cost or trigger provider-side rate-limit blocks on the user's account. **Mitigation deferred:** post-v1 — add `multi_model_review.rate_limits.{per_minute, per_hour, per_day}` config keys with sensible defaults (e.g., 10/minute, 100/hour, 1000/day) enforced by the orchestrator before fan-out.

These four limitations are intentionally NOT blockers for v1 (none represents a default-unsafe configuration; all require either a compromised system, deliberate misconfiguration, or operator intent). They are tracked as the canonical roadmap for ADR-003 follow-up hardening.
...
| 33 | Add `standing-pool-cleanup` to `plugin.json` agents array (extend from Task 8). **Coordinated PR with Tasks 8 and 36 to avoid `agents` array merge conflicts.** | S | Task 32; Task 8 | done — Added `standing-pool-cleanup` to `plugins/synthex-plus/.claude-plugin/plugin.json` agents array. Commit `41c5504`. |
...
| 36 | Add `standing-pool-submitter` to `plugin.json`. **Coordinated PR with Tasks 8 and 33.** | S | Task 35; Task 8 | done — Added `standing-pool-submitter` to `plugins/synthex-plus/.claude-plugin/plugin.json` agents array. Combined with Task 33 in same commit `41c5504`. |
...
| 38 | Layer 2 fixture at `tests/fixtures/multi-model-teams/routing/discovery-and-submit/`: spawn `code-reviewer + security-reviewer` pool; standard command needs same set (covers); inline discovery (Task 54) returns `routed-to-pool`; submitter writes 2 task files atomically, mailbox notification posted; pool processes, writes envelope to report-to path; submitter reads envelope; returns `status: success`. Asserts: UUID-based filenames; `.tmp`+rename pattern verifiable from filesystem trace; report-to uses unique-per-submission UUID. | L | Tasks 35, 37; Phase 3.2; Phase 5 (Task 41 spawns the pool) | done — Layer 2 fixture at tests/fixtures/multi-model-teams/submission/discovery-and-submit/. 56 tests in submission-fixture.test.ts, all passing. Exercises inline discovery → submitter invocation → success envelope → provenance line (NFR-MMT7 Item 4). Commit 1b4e3a0. |
| 39 | Layer 2 fixture at `tests/fixtures/multi-model-teams/routing/concurrent-submitters/`: two simulated submitting commands write to same pool concurrently. Asserts: both succeed; each receives correct envelope at its own report-to path (no cross-contamination); pool serializes via file-based task list (FR-MMT18 criterion). | M | Task 38 | done. Layer 2 fixture at tests/fixtures/multi-model-teams/submission/concurrent-submitters/. 47 tests in concurrent-submitters-fixture.test.ts, all passing. Models FR-MMT18: UUID-isolated report-to paths prevent collision across concurrent submitters. Commit 61d05ac. |
| 40 | Layer 2 fixture at `tests/fixtures/multi-model-teams/routing/timeout/`: spawn pool that intentionally hangs; submitter polls; after timeout, asserts: tasks marked `abandoned`; verbatim FR-MMT16a one-line note emitted; routing decision = `fell-back-timeout`. | M | Tasks 35, 37; Phase 5 | done — Layer 2 fixture at tests/fixtures/multi-model-teams/submission/timeout-fallback/. 26 tests in timeout-fixture.test.ts, all passing. Models poll timeout, task abandonment, verbatim FR-MMT16a §3.4 timeout note, fresh-spawn fallback. Commit af11dd0. |
| 30b | Layer 2 fixture at `tests/fixtures/multi-model-teams/pool-lifecycle/draining-end-to-end/` (split from former Task 30): spawn pool with one in-flight task; trigger TTL expiration; observe Pool Lead transitions to `draining` (verified separately by Task 30a); **submitter receives drain state and rejects new submission with `routing_decision: "fell-back-pool-draining"` per FR-MMT14a**; observe in-flight task completes; observe submitter's caller falls back per `prefer-with-fallback`. | M | Task 30a; Task 35; Phase 5 | done — Layer 2 fixture at tests/fixtures/multi-model-teams/submission/draining-rejection/. 22 tests in draining-rejection-fixture.test.ts, all passing. Models FR-MMT14a race condition: index shows idle, config.json shows draining; submitter drain check fires, returns fell-back-pool-draining without writing tasks. Commit 9c1bcd6. |
...
**Cost advisory verbatim copy** per D16/NFR-MMT2: `"Heads up: this pool will keep {reviewer_count} reviewers idle for up to {ttl_minutes} minutes. Estimated idle cost: ~{cost_estimate}. Continue?"` (interpolated; embed verbatim in command body and in Task 46(e) fixture). | L | Phase 3; parent Phase 3 (multi-model preflight) | done (iter 5) — 35 [T] tests in tests/schemas/start-review-team.test.ts. Test→criterion: [T1] steps 1–10 each verified; [T2] regex ^[a-z0-9][a-z0-9-]{0,47}$ verbatim; [T3] verbatim rejection message fragments; [T4] multi-model preflight + FR-MR20 ref; [T5] mkdir + .index.lock; [T6] pool_state: idle in step 8; [T7] last_active_at = spawn timestamp; [T8] .index.json.tmp+rename; [T9] verbatim cost advisory ("Heads up: this pool will keep", "idle for up to", "minutes", "Continue?"); [T10] submission_timeout_seconds in step 10; [T11] Standing Pool Identity Confirm Overlay in step 7; [T12] Standing Pool Lifecycle Overlay in step 7; [T13] lifecycle overlay Pool Lead only (not reviewer prompts); [T14] multi-model overlay gated on multi_model: true; [T15] "Pool Lead" terminology; [T16] pool roster scope note (planning roster / no routing in v1). |
| 42 | Author `plugins/synthex-plus/commands/stop-review-team.md` per FR-MMT10. Implements: parameter table (`name`, `all`, `force`); resolve target pools (interactive prompt via `AskUserQuestion` if no flags, with verbatim FR-MMT10 step 1 prompt); **when invoked with no args, display the standing-pools section of the `/list-teams` table format BEFORE the prompt**; in-flight task warning unless `--force`; send shutdown via `SendMessage` to Pool Lead with `type: shutdown`; wait up to 30s for cleanup with verbatim 30s-elapsed message per FR-MMT10 step 4; update index per Task 25 lock semantics; per-pool status confirmation. **Terminology:** "Pool Lead" only. | L | Phase 3 (Pool Lead drain logic — Task 27) | done (iter 5) — 28 [T] tests in tests/schemas/stop-review-team.test.ts. Test→criterion: [T1] three params (name/all/force); [T2] AskUserQuestion with verbatim FR-MMT10 prompt; [T3] no-args path: standing-pools table before prompt; [T4] in-flight warning unless --force; [T5] SendMessage type:shutdown to Pool Lead; [T6] verbatim 30s timeout message; [T7] .index.lock locking; [T8] force-cleanup hint with manual paths; [T9] "Pool Lead" terminology (positive+negative checks). |
| 43 | Author `plugins/synthex-plus/commands/list-teams.md` per FR-MMT11. Implements: enumerate non-standing teams from `~/.claude/teams/` (excluding `standing/`); enumerate standing pools from `~/.claude/teams/standing/index.json`; gather per-team metadata; display in two-section table (standing first, non-standing second) per FR-MMT11 verbatim; State column with four enumerated values; TTL Remaining always integer; State-value reference footnote in interactive output; friendly empty-list message. | M | Phase 3 | done (iter 5) — 25 [T] tests in tests/schemas/list-teams.test.ts. Test→criterion: [T1] both team types enumerated (2 tests); [T2] table columns Name/State/Roster/Multi-Model/Tasks/Idle/TTL Remaining (7 tests); [T3] State column + all four pool_state values (4 tests); [T4] TTL Remaining always integer (2 tests); [T5] state-value reference footnote (6 tests); [T6] empty-list friendly message (2 tests); [T7] standing pools section before non-standing (1 test). |
| 44 | Update `plugins/synthex-plus/.claude-plugin/plugin.json` to register all three new commands in `commands` array. **Single coordinated PR with Tasks 41–43.** Bump synthex-plus plugin version per CLAUDE.md release rules (deferred to Phase 9 Task 72). | S | Tasks 41, 42, 43 | done (iter 5) — plugin.json updated: start-review-team.md, stop-review-team.md, list-teams.md added to commands array. Existing 5 commands unaffected. JSON validates. Version bump deferred to Phase 9 Task 72 per plan. |
| 45 | Author Layer 1 schema validators: (a) `tests/schemas/start-review-team-output.ts` — confirmation output shape, cost-warning conditional per D16, `submission_timeout_seconds` row; (b) `tests/schemas/list-teams-output.ts` — table column structure, State enum (all four including `stopping`), TTL integer-ness; (c) `tests/schemas/stop-review-team-output.ts` — per-pool status messages, pre-prompt table display (no-args path). Vitest with normal and edge-path samples. | M | Tasks 41, 42, 43 | done (iter 5) — 85 [T] tests across 3 validator+test file pairs: start-review-team-output (28 tests): pool_state: "idle" enforcement, cost_warning_shown bool, reviewers non-empty, positive-integer ttl/timeout; list-teams-output (32 tests): all four pool_state enum values, ttl_remaining_minutes integer, tasks object, empty-array acceptance; stop-review-team-output (25 tests): STOP_RESULT_VALUES enum, pre_prompt_table_shown bool, pools array, optional message. Exports: validateStartReviewTeamOutput, validateListTeamsOutput, validateStopReviewTeamOutput, SPAWN_POOL_STATE_VALUES, POOL_STATE_VALUES, STOP_RESULT_VALUES. |
| 46 | Layer 2 fixtures at `tests/fixtures/multi-model-teams/commands/`: (a) `start-default/` — verifies `config.json` and `index.json` written; **pool teammates' composed spawn prompts contain `### Standing Pool Identity Confirm Overlay` verbatim**. (b) `start-multi-model/` — verifies preflight (D15) and warning-level results trigger Continue prompt; **Pool Lead spawn prompt contains all three overlays verbatim; pool reviewer spawn prompts contain identity-confirm + multi-model verbatim, but NOT Lifecycle**. (c) `start-invalid-name/` — regex-validation reject before any FS write. (d) `start-duplicate-name/` — duplicate-name abort with FR-MMT9 hint. (e) `start-cost-warning/` — 4-reviewer pool triggers D16 cost advisory (raw-string match on Task 41 verbatim). (f) `stop-with-inflight/` — `--force` overrides in-flight warning. (g) `list-empty/` — friendly empty message. (h) `list-with-mixed-states/` — State column renders for ALL FOUR `pool_state` values; **sub-scenario:** a fixture with a pool whose `config.json` has `pool_state: stopping` exercises the live `list-teams` path. (i) `stop-no-args-interactive/` — invokes `/stop-review-team` with no args; standing-pools table displayed BEFORE prompt; user picks pool; pool stops. (j) `stop-no-args-cancel/` — same as (i); user cancels; verifies no changes (index.json unchanged, no shutdown signal). | L | Tasks 41–45 | done (iter 5) — 74 [T] tests in tests/schemas/command-fixtures.test.ts across 10 fixture directories in tests/fixtures/multi-model-teams/commands/. Test→criterion: (a) start-default: idle state + Identity Confirm Overlay verbatim + no cost warning; (b) start-multi-model: Pool Lead gets all 3 overlays, reviewers get Identity+MultiModel NOT Lifecycle (negative check); (c) start-invalid-name: verbatim rejection message + no FS write; (d) start-duplicate-name: remediation hints; (e) start-cost-warning: verbatim D16 advisory + cost_warning_shown=true; (f) stop-with-inflight: --force suppresses warning + force_stopped result; (g) list-empty: friendly empty message; (h) list-with-mixed-states: all 4 pool_state values + draining/stopping ttl=0; (i) stop-no-args-interactive: table before prompt + pre_prompt_table_shown=true; (j) stop-no-args-cancel: index_changed=false + shutdown_sent=false. |
...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 47 | Implement FR-MMT22 stale-pool cleanup as a sub-section in **Task 31's `routing.md` (`## Stale-Pool Cleanup`)** that calls into the cleanup-only Haiku agent (Task 32). Handles both detection conditions (metadata dir missing OR `last_active_at` older than `max(ttl_minutes, 24h)`): inline discovery in Tasks 54/57 detects; invokes `standing-pool-cleanup`; emit one-time-per-session warning per **verbatim FR-MMT22 step 5 message: `"Standing pool '{name}' was stale and has been cleaned up. {fallback_action}."`** (embed verbatim in command body — distinct from FR-MMT28 orphan warning). Continues with routing decision per `routing_mode`. **Note: orphan-scan path (Task 49) reuses this cleanup but emits FR-MMT28 warning instead.** | M | Tasks 31, 32; Phase 3.1 | done (iter 5) — 15 [T] tests in tests/schemas/stale-cleanup-routing.test.ts. Test→criterion: [T1] ## Stale-Pool Cleanup section heading; [T2] standing-pool-cleanup agent reference; [T3] index entry + metadata dir removal under lock; [T4] "transient marker in the calling session's state" verbatim; [T5] FR-MMT22 verbatim warning fragments; [T6] FR-MMT22 vs FR-MMT28 distinct warning strings; [T7] detection conditions (metadata_dir + last_active_at); [T8] max(ttl_minutes, 24h) floor; [T9] routing continues per routing_mode after cleanup; [T10] FR-MMT22 reference. |
| 48 | **Reframe per Architect cycle 1 finding:** FR-MMT24 per-task fallback recovery is the **submitting command's host session** responsibility (NOT `standing-pool-submitter.md`). Submitter's responsibility ends at returning the envelope (Task 35). This task: (1) authors shared spec at `docs/specs/multi-model-teams/recovery.md` documenting FR-MMT24 verbatim including: failed-reviewer-name extraction from envelope `error.message`; spawn fresh native sub-agent for that reviewer only via Task tool in submitting command's host session (same way command would spawn in non-pool mode); wait for fresh sub-agent's findings (markdown + `findings_json` if multi-model); lightweight merge per FR-MMT24 step 4 — append to surviving findings, NO full re-consolidation; for multi-model pools, run partial dedup per D19 — Stages 1+2 from parent reused as "partial dedup" entry point; emit unified report with FR-MMT24 step 5 header and `source.source_type: "native-recovery"` for recovered findings. (2) Adds workflow steps to `review-code.md` (Task 54) and `performance-audit.md` (Task 57) referencing `recovery.md`; recovery invoked when envelope `status: failed` with `error.code: reviewer_crashed`. **Skeleton includes all normative procedure from the start.** Begins with `## Status: Skeleton`. | L | Task 35; parent Phase 3.2 (Stages 1, 2 reusable — reference exact parent task numbers in PR description) | done (iter 5) — 16 [T] tests in tests/schemas/recovery-md.test.ts. Test→criterion: [T1] ## Status: Skeleton; [T2] host-session recovery ownership; [T3] error.message reviewer-name extraction; [T4] Task tool spawn from host session; [T5] Stages 3+ NOT re-run; [T6] D19 Stage 1+2 + parent plan ref; [T7] source.source_type: "native-recovery" verbatim; [T8] verbatim FR-MMT24 step-5 header (3 fragments); [T9] standing-pool-submitter NOT owning recovery; [T10] pool_lead_crashed/drain_timed_out as terminal; [T11] review-code.md + performance-audit.md as integration points. |
| 49 | Update synthex-plus's one-team-per-session check (currently in `team-implement.md:77-82` and equivalents) to count only non-standing teams per FR-MMT26. Exempt paths under `~/.claude/teams/standing/`. Update orphan-detection scanner per FR-MMT28 to: (a) exclude `~/.claude/teams/standing/` from existing one-team-per-session orphan check; (b) add separate scan of `~/.claude/teams/standing/` applying standing-pool orphan rule (BOTH TTL elapsed AND `last_active_at` more than 24h old); orphan scan reuses Task 47's cleanup but emits the **verbatim FR-MMT28 standing-pool orphan warning** (a distinct verbatim string from FR-MMT22 in Task 47 — do NOT reuse FR-MMT22's wording). **Cross-reference Task 47:** confirm distinct strings; document that FR-MMT22 (in-session stale) and FR-MMT28 (init-time orphan) emit different verbatim warnings. **Suppression scope:** FR-MMT22's one-time-per-session suppression does NOT suppress orphan warnings during `team-init` scan. | M | Phase 3, Task 47 | done (iter 5) — 14 [T] tests in tests/schemas/one-team-exemption.test.ts. Test→criterion: [T1] standing-pool exemption in team-implement.md; [T2] FR-MMT26 ref; [T3] original error message unchanged (3 verbatim fragments); [T4] Pass 1 excludes standing/ path; [T5] Pass 2 dual-condition (TTL elapsed + 24h inactive, both required); [T6] standing-pool-cleanup invoked in orphan path; [T7] FR-MMT28 verbatim warning (3 fragments); [T8] FR-MMT28 warning ≠ FR-MMT22 wording (negative check); [T9] FR-MMT22 suppression does not suppress FR-MMT28; [T10] FR-MMT28 ref. |
| 50 | Layer 2 fixture at `tests/fixtures/multi-model-teams/failure-handling/stale-pool/`: (a) metadata dir missing → cleanup runs, verbatim FR-MMT22 step 5 warning; (b) `last_active_at` more than 24h ago → cleanup runs, verbatim warning; (c) warning fires once per session per stale pool (run discovery twice, verify only one warning); (d) `prefer-with-fallback` mode continues without erroring; (e) `explicit-pool-required` mode aborts after cleanup leaves no matching pool; (f) FR-MMT22 in-session warning vs FR-MMT28 orphan-scan warning emit distinct strings (raw-string check both, assert non-equal). | L | Task 47; Task 49 | done (iter 5) — 55 [T] tests in tests/schemas/stale-pool.test.ts across 6 sub-scenarios. Test→criterion: (a) FR-MMT22 Condition 1 metadata-dir-missing → cleanup → verbatim warning fragments; (b) Condition 2 last_active_at stale (>max(ttl, 24h)) → cleanup → warning; (c) session-marker suppresses duplicate warning same pool, separate pool gets own warning; (d) prefer-with-fallback: routing_decision=fell-back-no-pool, error_shown:false; (e) explicit-pool-required: abort:true, error_text contains "No standing pool matches"; (f) FR-MMT22 warning !== FR-MMT28 warning (strict non-equality). |
| 51 | Layer 2 fixture at `tests/fixtures/multi-model-teams/failure-handling/recovery-native-only/`: native-only pool with `code-reviewer + security-reviewer`; code-reviewer crashes mid-task; security-reviewer completes; submitter receives `status: failed`; **submitting command's host session** spawns fresh `code-reviewer` per `recovery.md` (Task 48); merges via simple append; unified report header indicates recovery; recovered findings carry `source.source_type: "native-recovery"`. | M | Task 48; Tasks 54, 57 | done (iter 5) — 38 [T] tests in tests/schemas/recovery-native-only.test.ts. Test→criterion: [T1] surviving_findings=3 preserved; [T2] source_type:"native-recovery" on fresh reviewer; [T3] verbatim FR-MMT24 step-5 header; [T4] invoked_by:"submitting_command_host_session" + Task-tool spawn; [T4b] submitter_owns_recovery:false; [T5] stages_3_to_6_run:false; [T6] total_findings=surviving+recovered. |
| 52 | Layer 2 fixture at `tests/fixtures/multi-model-teams/failure-handling/recovery-multi-model/`: multi-model pool 2 native + 2 external; native code-reviewer crashes; submitting command triggers recovery per `recovery.md`; fresh native code-reviewer produces markdown + `findings_json`; D19 partial dedup (Stages 1+2 only) runs against already-consolidated output; recovered findings either merged (gaining `native-recovery` attribution) or emitted as new entries. Asserts: Stage 3+ NOT re-run (verifiable by absence of CoVe LLM calls in trace). | M | Task 48; parent Phase 3.2 | done (iter 5) — 45 [T] tests in tests/schemas/recovery-multi-model.test.ts. Test→criterion: [T1] D19 partial dedup stages_run=[1,2], stages_not_run=[3,4,5,6]; [T2] cove_llm_calls=0; [T3] source_type:"native-recovery"; [T4] full_reconsolidation_run:false; [T5] verbatim recovery header; [T6] has_findings_json:true (markdown+findings_json); [T7] stages_run.length(2) < total(6). |
| 53 | Layer 2 fixture at `tests/fixtures/multi-model-teams/failure-handling/one-team-per-session/`: spawn standing pool; spawn `/team-review` (non-standing); both succeed (FR-MMT26 criterion 2); spawn second `/team-review`; aborted with original error (FR-MMT26 criterion 3, message unchanged). | S | Task 49 | done (iter 5) — 20 [T] tests in tests/schemas/one-team-per-session.test.ts. Test→criterion: [T1] standing pool + non-standing coexist (spawn_allowed:true); [T2] second non-standing aborts (spawn_allowed:false); [T3] original error message unchanged ("Error: An active team" prefix); [T4] standing pool unaffected by conflict (affected_by_conflict:false). |
...
Discovery is conditional on `standing_pools.enabled: true` from `.synthex-plus/config.yaml`.

_Done. Inserted Step 1b into review-code.md with inline discovery (FR-MMT15), FR-MMT17 routing notification, pool-submitter invocation, FR-MMT24 recovery on reviewer_crashed. 17 tests in review-code-routing.test.ts. Commit 35fac7e._ | L | Phase 4, Phase 5, Phase 6 | done |
...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 57 | Update `plugins/synthex/commands/performance-audit.md` to add discovery + routing + recovery from Task 54 (same pattern; INLINE). Required-reviewer-set is **static** per FR-MMT15 normative — hardcoded `[performance-engineer]`; no `--reviewers` flag, no `performance_audit.reviewers` config. Same flow as Task 54; same recovery via `recovery.md` (Task 48). NFR-MMT7 output template applied per Task 55 — **identical verbatim copy reused for all four items** (NOT independently authored — see D25). _Done. Inserted Step 1b into performance-audit.md with static required set [performance-engineer], same routing/recovery pattern as Task 54, verbatim NFR-MMT7 strings reused from Task 55 (D25). Cross-file equality verified. 19 tests in performance-audit-routing.test.ts. Commit 74b290f._ | M | Tasks 54, 55; Phase 5 | done |
| 58 | Layer 2 fixtures at `tests/fixtures/multi-model-teams/routing/performance-audit/`: (a) `pool-match/` — pool with `performance-engineer` (single-reviewer roster, `covers` match against `[performance-engineer]`); routes; receives report. (b) `no-pool-fallback/` — no matching pool; silent fall-through; report byte-identical to baseline (Task 0). (c) `roster-mismatch/` — pool with only `code-reviewer + security-reviewer` doesn't cover `[performance-engineer]`; falls back. _Done. 3 scenario fixtures under tests/fixtures/multi-model-teams/routing/performance-audit/. 50 tests in performance-audit-fixtures.test.ts, all passing. Commit 9c13961._ | M | Task 57; Phase 5 | done |
| 34a | **Deferred status — runs after Phase 7 ships.** Full end-to-end Layer 2 latency fixture at `tests/fixtures/multi-model-teams/routing/latency/` asserting NFR-MMT3 across the live submitting-command path. Three sub-cases: 0/1/10 indexed pools. Asserts: discovery step (inline read + filter) < 100 ms across all sub-cases; end-to-end cold-case routing overhead with `standing_pools.enabled: true` < 500 ms (full path through `/review-code` and `/performance-audit`). Records CI-stable methodology (wall-clock around discovery substep AND full routing decision, multiple runs, P95 reported). Complements Task 34a-pre (discovery-primitive layer, Phase 4); this closes the end-to-end NFR-MMT3 contract once consumer commands exist. | M | Tasks 32, 54, 57 | done. End-to-end latency fixture at tests/fixtures/multi-model-teams/routing/latency/end-to-end/. 3 sub-cases (0/1/10 pools). 19 tests in latency-end-to-end-fixture.test.ts, all passing. Projected P95 values within NFR-MMT3 budgets (discovery < 100ms, end-to-end < 500ms). Commit 6336269. |
...
## Phase 8 — Audit Extensions, Discoverability, Documentation — **COMPLETE**
...
**Skip default:** Both new prompts default to Skip (Enter without text = Skip). **Non-interactive (CI):** TTY check analogous to NFR-MMT7 item 3 — when stdout is not a TTY, both prompts auto-select Skip without blocking. The "Available templates" section is unchanged. | M | Phase 5; parent Phase 6 (init updates) | Done. Added Steps 7+8 to team-init.md (inserted after Step 6, original Step 7 renumbered to 9): Step 7 adds standing-pool Enable/Skip prompt + standing_pools.enabled config; Step 8 adds multi-model team-review Enable/Skip; Step 9 updated with 3 verbatim pool management commands when standing_pools.enabled. 10 tests in team-init-fr-mmt27.test.ts. Commit 9af2f53. |
| 64 | Layer 2 fixture at `tests/fixtures/multi-model-teams/init/`: (a) skip both → config unchanged. (b) enable standing pools, skip multi-model → `standing_pools.enabled: true` written; no pool spawned (FR-MMT27 criterion 3); Step 7 lists three new commands with verbatim descriptions. (c) enable both → both config keys written; Step 7 updated. (d) non-interactive (TTY=false) → both prompts auto-skip; no config changes; no blocking. (e) interactive default (Enter without text) → prompt treats as Skip. Done. 5 scenario fixtures under tests/fixtures/multi-model-teams/init/: skip-both, enable-standing-pools-skip-multi-model, enable-both, non-interactive-tty-false, interactive-default-enter. 42 tests in team-init-fixtures.test.ts, all passing. Commit c0a1afd. | M | Task 63 | done |
| 65 | Final pass on `docs/specs/multi-model-teams/architecture.md` (replaces Task 3 skeleton per `## Status: Final`). Adds narrative prose, examples, cross-references to normative content already in skeleton. Covers: Option B, two-consolidation-surfaces (FR-MMT4), bridge mechanism (FR-MMT20), cross-session lifetime (FR-MMT5a), identity drift (FR-MMT5b), state machine, audit-artifact extensions, deferred-Stage-3 inheritance from parent. Cross-references to parent's architecture doc. **Related documentation** section at top with links to `pool-lifecycle.md`, `routing.md`, `recovery.md`, `standing-pools.md`. | L | Phase 6 | done — `docs/specs/multi-model-teams/architecture.md` — `## Status: Final`, Related Documentation section with all 4 links, narrative prose for all 8 architectural concerns (D3, FR-MMT4/20/5a/5b, FR-MMT22, D26, FR-MMT30/30a). 3517 tests pass. Commit. |
| 66 | Final pass on `docs/specs/multi-model-teams/pool-lifecycle.md` (replaces Task 22 skeleton). Adds narrative prose, examples, cross-references. Covers: per-pool `config.json` schema (FR-MMT7), index entry schema (FR-MMT9b), state machine, TTL semantics (FR-MMT13), draining (FR-MMT14a), stale-pool detection (FR-MMT22), recovery (cross-reference `recovery.md`), one-team-per-session exemption (FR-MMT26), orphan classification (FR-MMT28). Related documentation section at top. | L | Phase 6 | done — pool-lifecycle.md skeleton replaced: removed skeleton marker, added Overview, State Machine Quick Reference, Locking Quick Reference, updated cross-references, added Worked Example (pool startup → task claim → idle hook → shutdown). Commit bc23737. |
| 67 | Final pass on `docs/specs/multi-model-teams/routing.md` (replaces Task 31 skeleton). Adds narrative prose, examples, cross-references. Covers: discovery (FR-MMT15 with v1 scope per D10, INLINE-discovery convention), submission (FR-MMT16), envelope (FR-MMT16a), prefer-with-fallback (FR-MMT17), explicit-pool-required, race conditions (FR-MMT18), output template (NFR-MMT7), v2 extension points per D10. Related documentation section at top. _Done. routing.md skeleton replaced with production doc: added Overview, Quick Reference table (7 routing decisions), updated cross-references, added Tutorial §8 with 3 scenarios. 19 tests in routing-md.test.ts pass. Commit 407b979._ | L | Phase 7 | done |
| 67a | Final pass on `docs/specs/multi-model-teams/recovery.md` (replaces Task 48 skeleton). Covers: trigger (envelope `status: failed` with `error.code: reviewer_crashed`), failed-reviewer-name extraction, fresh sub-agent spawn from submitting command's host session via Task tool, lightweight merge for native-only, D19 partial dedup (Stages 1+2) for multi-model, `source.source_type: "native-recovery"` attribution, FR-MMT24 step 5 report header. Related documentation section at top. _Done. recovery.md skeleton replaced: added Overview, Recovery Decision Tree, updated cross-references (Tasks 54/57 now done), added Worked Example §5 with security-reviewer crash scenario. 15 tests in recovery-md.test.ts pass. Commit 0511b12._ | M | Phase 6 | done |
| 68 | Author user-facing design doc at `plugins/synthex-plus/docs/standing-pools.md` per NFR-MMT8. Audience: end-users. Covers: what standing pools are, when to use, three commands with examples, configuration overview, multi-model variant, troubleshooting (orphans, stale pools, manual cleanup). Cross-references all four `docs/specs/multi-model-teams/*.md` docs. Includes empirical idle-cost measurement placeholder per NFR-MMT2 criterion 1 (filled after dogfooding per PRD §7.1a). | L | Tasks 65, 66, 67, 67a | done — `plugins/synthex-plus/docs/standing-pools.md` — user-facing design guide with 3-command walkthrough, configuration overview, multi-model variant, troubleshooting, idle-cost placeholder per NFR-MMT2. Cross-refs to all 4 spec docs. Commit `6a49441`. |
| 69 | Update `plugins/synthex-plus/README.md` per NFR-MMT8: add commands and link to `docs/standing-pools.md`; one-paragraph overview of Feature A and Feature B; off-by-default statement; cross-reference to architecture doc. | S | Task 68 | done — `plugins/synthex-plus/README.md` updated — v0.2.0 section with Feature A/B overview, `start-review-team`/`stop-review-team`/`list-teams` commands, off-by-default statement, `standing-pools.md` link, architecture doc cross-ref. |
| 70 | Update `plugins/synthex/README.md` per NFR-MMT8: note pool routing in `/review-code` and `/performance-audit`; cross-reference to standing-pools doc; explicit "off by default unless synthex-plus is initialized with `standing_pools.enabled: true`" statement. | S | Task 68 | done — `plugins/synthex/README.md` updated — Standing Review Pools section documenting pool routing in `/review-code` and `/performance-audit`, verbatim off-by-default statement, `standing-pools.md` link. Commit `7b92a12`. |
| 71 | Update root `CLAUDE.md` per NFR-MMT8: add `start-review-team`, `stop-review-team`, `list-teams` to synthex-plus commands table; describe pool routing in standard commands; reference new doc directory. | S | Task 68 | done — Root `CLAUDE.md` updated — `start-review-team`, `stop-review-team`, `list-teams` in synthex-plus commands table; pool routing note under standard commands; `docs/specs/multi-model-teams/` directory reference. |
...
**Status:** All of Phases 3–9 (Tasks 33, 36, 65, 68–75) complete. Only **Phase 10 (Tasks 76–78, Layer 3 evals)** remains pending — deferred per **D24** (NFR-MMT5 wall-clock parallelism verification deferred post-v1).

### Milestone 9.1: Version bumps, changelog, dogfooding handoff

| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 72 | Bump `plugins/synthex-plus/.claude-plugin/plugin.json` version per CLAUDE.md release rules. | S | Phase 8 | done — `plugins/synthex-plus/.claude-plugin/plugin.json` version bumped 0.1.2 → 0.2.0. Commit `74deabe`. |
| 72a | **Bump `plugins/synthex/.claude-plugin/plugin.json` version** per CLAUDE.md — required because Tasks 54 and 57 modified `plugins/synthex/commands/review-code.md` and `plugins/synthex/commands/performance-audit.md`. Without this bump, users on the base Synthex plugin won't see an upgrade signal. | S | Phase 8 | done — `plugins/synthex/.claude-plugin/plugin.json` version bumped 0.5.0 → 0.5.1. Commit `74deabe`. |
| 73 | Bump top-level `.claude-plugin/marketplace.json` version AND **BOTH the synthex-plus AND synthex entries' versions** per CLAUDE.md (both plugins changed). | S | Tasks 72, 72a | done — `.claude-plugin/marketplace.json` top-level version 0.5.0 → 0.5.1; synthex-plus entry 0.1.2 → 0.2.0; synthex entry 0.5.0 → 0.5.1. Commit `dd2a6e5`. |
| 74 | Add `CHANGELOG.md` entry covering all FR-MMT features shipped in v1, with link reference at the bottom. Mentions: standing-pool functionality (3 new synthex-plus commands), multi-model on `/team-review`, **pool routing for `/review-code` and `/performance-audit` (in synthex base plugin)**, audit extensions (team/pool/recovery/attribution), `team-init` updates. **Mentions `~/.claude/teams/standing/` storage path.** Note: the `mkdir -p ~/.claude` hint is included only if pre-flight verification (LOW finding) confirms Claude Code does not guarantee `~/.claude/` exists at command runtime; otherwise omitted. | S | Tasks 72, 72a, 73 | done — `CHANGELOG.md` entries added: [synthex-plus 0.2.0] covering Feature A/B, audit extensions, new commands, `~/.claude/teams/standing/` storage path; [0.5.1] covering pool routing in synthex base. Off-by-default statement present. `mkdir -p ~/.claude` hint omitted. |
| 75 | Set up dogfooding cohort per PRD §7.1a contract. Owner: PM. Coordinate ≥ 3 internal users; ≥ 2 weeks sustained usage; create empty `docs/retros/multi-model-teams-dogfooding.md` with measurement template (Pool routing hit rate, Wall-clock speedup, Multi-model finding-quality lift, Adoption rate, Fallback rate, Cross-session pool usage per PRD §7.2). Tech-lead-owned: confirm audit-driven measurements (FR-MMT30 routing decision enum, FR-MMT30a finding attribution telemetry) capture data automatically. | M | Phase 8 | done — `docs/retros/multi-model-teams-dogfooding.md` created with 6 PRD §7.2 metrics measurement template, instrumentation confirmation table (all 6 metrics captured automatically via audit artifacts), cohort placeholder for ≥3 users. Commit `497aa78`. |
...
**Task 79 Completion Notes:** Done. D27 renamed to `D27 / ADR-003` and body updated with verbatim Pattern 1/2/3 descriptions. `multi_model_review.external_permission_mode.default: read-only` added to `plugins/synthex/config/defaults.yaml` with full inline docs for all three patterns. `[T]` ADR-003 criterion: `mmt-defaults-yaml-task79.test.ts` → "plan Decisions table contains ADR-003 identifier" + "ADR-003 and all three pattern descriptions appear in the same D27 row". `[T]` config-key criterion: `mmt-defaults-yaml-task79.test.ts` → "multi_model_review.external_permission_mode.default is read-only". `[H]` inline docs reviewed: each pattern (read-only, parent-mediated, sandbox-yolo) has a prose explanation of when to use it; approval-proxying and confirmation-required semantics are documented. 9 new tests in `tests/schemas/mmt-defaults-yaml-task79.test.ts`; all pass. Full schema suite: 3557 pass, 5 pre-existing failures in `release-v050.test.ts` (unrelated). Commit TBD.
...
**Task 80 Completion Notes:** Done. `plugins/synthex/agents/codex-review-prompter.md` now defaults to Pattern 3 (parent-mediated) via `codex app-server --json`. New `## Permission Model (ADR-003 / D27)` section documents the three-pattern table and `multi_model_review.external_permission_mode.codex` config key. New `## requestApproval Proxying (Pattern 3)` section documents the JSON-RPC envelope format, the `codex-approval-request` fenced-block surface to the parent, and the round-trip stdin response. Step 4 (CLI Invocation) is now mode-aware with branching for `parent-mediated` / `read-only` / `sandbox-yolo`. Pattern 1 (`codex exec --sandbox read-only --approval-mode never`) preserved as fallback when `codex app-server --help` exits non-zero. Test linkage (`tests/schemas/codex-adapter-md.test.ts`): `[T]` app-server flag → "[T] uses app-server JSON-RPC mode (raw-string check for app-server invocation flag)"; `[T]` requestApproval parsing → "[T] documents requestApproval JSON-RPC parsing (raw-string check)" + "[T] documents the requestApproval proxy mechanism to parent session"; `[T]` Pattern 1 fallback → "[T] documents fallback to Pattern 1 when app-server unavailable (raw-string check)". 38 tests pass (8 new for Task 80). Commit `011c291`.
...
**Task 81 Completion Notes:** Done. Four adapters updated with a uniform `## Permission Model (ADR-003 / D27)` section: `gemini-review-prompter.md` (real `--readonly` flag), `bedrock-review-prompter.md`, `llm-review-prompter.md`, `ollama-review-prompter.md` (no flag — explicit "no tool-use surface" safety rationale per HTTP/stateless protocol). Each adapter now references `multi_model_review.external_permission_mode.<cli-name>`, the literal `sandbox-yolo` mode, and rejects `parent-mediated` with `error_code: cli_unsupported_mode` (only Codex and Claude Code support that). Test linkage (`tests/schemas/external-adapter-permission-model.test.ts`): `[T]` read-only flag/rationale → "[T] documents read-only flag or explicit safety rationale" (×4, one per adapter); `[T]` config-read step for sandbox-yolo → "[T] references the external_permission_mode config key (raw-string check for sandbox-yolo override step)" + "[T] references the sandbox-yolo mode literally" (×4 each). 32 new tests pass; 173 pre-existing per-adapter tests still pass; full focused suite 243 pass. Commit `3899755`.
...
**Task 82 Completion Notes:** Done. The Task 79 `external_permission_mode` block in `plugins/synthex/config/defaults.yaml` is now populated with real per-CLI defaults (replacing the placeholder commented examples): `codex: parent-mediated`, `claude: parent-mediated`, `gemini/bedrock/llm/ollama: read-only`, plus universal `default: read-only`. CLI-name convention is the bare adapter name (e.g., `codex`, not `codex-review-prompter`) — matching the convention adopted in Task 81's adapter docs. Inline rationale comment per entry. Test linkage (`tests/schemas/mmt-defaults-yaml-task82.test.ts`): `[T]` all CLI names → "[T] config block has entries for all six v1 CLI names" (6 sub-tests, one per CLI); `[T]` codex+claude default → "[T] Codex and Claude Code default to parent-mediated" (2 sub-tests); `[T]` others default → "[T] all other CLIs default to read-only" (4 sub-tests). Plus enum validation across all entries. 21 new tests pass; full focused suite (this + Task 79 + parent MMR/MMT defaults) 72 pass. Commit `cb1ec77` (or successor SHA after merge).
...
**Task 83 Completion Notes:** Done. Three commands updated with a uniform sandbox-yolo confirmation step: `plugins/synthex-plus/commands/start-review-team.md` (new Step 5a between preflight and lock acquisition); `plugins/synthex/commands/review-code.md` (new Step 1c between pool routing and the multi-model decision framework); `plugins/synthex/commands/performance-audit.md` (new Step 1c between pool routing and audit scope). Verbatim warning per D25/NFR-MMT7: `⚠ <cli-name> is configured in sandbox-yolo mode — CLI will run with full tool permissions inside an OS sandbox.` Followed by `[y/N]` prompt with Default-N semantics (Enter = abort). The check is a no-op when no CLI in the resolved roster has sandbox-yolo configured (default safe state). Test linkage (`tests/schemas/sandbox-yolo-confirm.test.ts`): `[T]` verbatim warning → "[T] verbatim warning string matches per D25 / NFR-MMT7" (×3 — one per command); `[T]` y/N confirmation prompt → "[T] y/N confirmation prompt is required (default N) for sandbox-yolo" (×3); `[T]` skipped for read-only/parent-mediated → "[T] confirmation step is skipped when no CLI is sandbox-yolo (only triggers for that mode)" (×3). 21 new tests pass; 48 pre-existing per-command tests still pass.

**Task 84 Completion Notes:** Done. Closes Phase 11.1. Three [T] criteria satisfied by `tests/schemas/permission-model-fixtures.test.ts` + three fixture trees: (1) Layer 1 enum validator → "Task 84 [T] (1): Layer 1 schema — external_permission_mode enum per CLI" (5 + 6 enum-per-CLI sub-tests); (2) `tests/fixtures/multi-model-review/adapters/codex/app-server-request-approval/` (4 files: fixture.json, expected-envelope.json, scenario.md, recorded-cli-invocation.txt) → "Task 84 [T] (2): Layer 2 fixture — Codex app-server requestApproval proxied to parent" (6 sub-tests); (3) `tests/fixtures/multi-model-review/adapters/gemini/read-only/` (4 files) → "Task 84 [T] (3): Layer 2 fixture — Gemini --readonly invocation (no destructive tool-use)" (6 sub-tests). Bonus fixture `tests/fixtures/multi-model-teams/sandbox-yolo-confirmation/` pins the Task 83 contract end-to-end (resolved-permission map, verbatim warning, context-appropriate prompts, default-N abort). 29 tests pass total.
...
**Task 85 Completion Notes:** Done. Three edits to `codex-review-prompter.md`: (a) probe caching sentence added to Step 4 (eliminates ~50–150ms per redundant probe; standing-pool fan-outs no longer accumulate N probes); (b) new `### Performance characteristics` subsection in requestApproval Proxying quantifying O(N) round-trip × 500ms–2s vs Pattern 1's O(1), recommending Pattern 1 for latency-sensitive contexts; (c) JSON-RPC id correlation rule embedded in step 3 of the proxy flow ("MUST verify `response.id == pending_request.id`"; on mismatch drop + WARN + continue). New Layer 2 fixture `tests/fixtures/multi-model-review/adapters/codex/app-server-id-mismatch/` with fixture.json + scenario.md covering the drop-mismatched-then-write-correlated path. Note: dropped `expected-envelope.json` from the fixture set (acceptance criterion language was inherited from the success-path fixture pattern; for the mismatch path the canonical envelope is the same as success — what's distinctive is the drop behavior, which is asserted directly against the fixture's `expected_adapter_behavior` block). Test linkage (`tests/schemas/codex-adapter-task85.test.ts`): 23 new tests covering all 5 [T] criteria; 61 with codex-adapter-md.test.ts regressions.
...
**Task 86 Completion Notes:** Done. `gemini-review-prompter.md` Step 4 split into 4a (probe), 4b (invocation with selected flag), 4c (sandbox_violation detection). Probe runs `gemini --help`, prefers `--readonly` over `--no-tools`, returns `cli_failed` with remediation if neither present. sandbox_violation detection is best-effort on observed write-tool evidence in output. Mirrors codex `app-server --help` precedent. Test linkage (`tests/schemas/gemini-adapter-task86.test.ts`): 12 new tests across 3 [T] criteria; 84 across this + regressions.
...
**Task 87 Completion Notes:** Done. Two new config keys (`sandbox_profile_path` defaulting to `plugins/synthex/config/sandbox.sb`; `sandbox_bwrap_flags` defaulting to `--ro-bind / / --bind /tmp /tmp --proc /proc --dev /dev`) added to defaults.yaml under `multi_model_review`. New file `plugins/synthex/config/sandbox.sb` ships a deny-by-default macOS sandbox-exec profile: denies all network egress, denies writes outside /tmp, denies reads of `~/.ssh`/`~/.aws`/`~/.config` even within CWD; uses `CWD_PATH` + `HOME_PATH` parameters passed via `sandbox-exec -D`. Both `codex-review-prompter.md` and `gemini-review-prompter.md` Pattern 2 sections now contain a `### Step 0 — Pattern 2 profile-existence check` subsection: macOS `test -r <sandbox_profile_path>` + Linux `which bwrap` validation; on failure returns `cli_failed` with a verbatim remediation message ("Pattern 2 (sandbox-yolo) sandbox profile not found at..."); explicit clarification that Step 0 only applies to Pattern 2 (Patterns 1 + 3 skip it). Pattern 2 invocation lines updated to pass `-D CWD_PATH=$PWD -D HOME_PATH=$HOME -f <sandbox_profile_path>`. Test linkage (`tests/schemas/sandbox-profile-task87.test.ts`): 25 new tests across 5 [T] criteria; 112 with regressions including codex-task85, gemini-task86, mmt-defaults-yaml-task82, permission-model-fixtures.
...
**Task 88 Completion Notes:** Done. Three layers of CWE-20 defense-in-depth: (1) `defaults.yaml` `external_permission_mode` block now has an inline comment block documenting the allow-list explicitly, stating unknown keys are silently ignored, cross-referencing the Layer 1 validator test. (2) All six adapter agents (codex, claude, gemini, bedrock, llm, ollama) gain a "Safe-name assertion" paragraph between Step 1 and Step 2 asserting the binary name is HARDCODED in the `which` invocation (never derived from config keys); bedrock notes its binary is `aws` (wraps `aws bedrock-runtime`). (3) New test `tests/schemas/external-permission-mode-key-validation.test.ts` enforces the allow-list + per-adapter assertions. Test linkage: `[T]` (1) enum-key validation → 'Task 88 [T] (1)' (5 sub-tests + 7 enum-key parametrized); `[T]` (2) defaults.yaml inline comment → 'Task 88 [T] (2)' (5 sub-tests); `[T]` (3) per-adapter safe-name assertion → 'Task 88 [T] (3)' (7 sub-tests × 6 adapters = 42). 58 new tests pass; 375 across all 6 adapter regressions + permission-model-fixtures + mmt-defaults-yaml-task82.
...
**Task 89 Completion Notes:** Done. Verbatim non-TTY guard sentence inserted right after "Default is **N**" in each of: start-review-team Step 5a, review-code Step 1c, performance-audit Step 1c. Test linkage (`tests/schemas/sandbox-yolo-tty-guard.test.ts`): 12 new tests including byte-identical extraction across files; 34 with regressions.
...
**Task 90 Completion Notes:** Done. New `### ADR-003 Known Limitations (out of scope for v1)` subsection inserted between the Decisions table and `## Open Questions`; D27 row cross-references it. Four gaps enumerated with descriptions + concrete "Mitigation deferred: post-v1" proposals. **`[H]` approval obtained 2026-04-29: "Approve as-is".** Test linkage (`tests/schemas/adr-003-known-limitations.test.ts`): 13 new tests covering presence, all four keywords (case-insensitive), out-of-scope framing, and D27-row regression.
...
**Task 91 Completion Notes:** Done. New module-level singleton at `tests/helpers/load-defaults.ts` exporting `loadDefaultsYaml()` (parsed, async, yaml→js-yaml fallback), `loadDefaultsYamlText()` (raw text), `getDefaultsYamlPath()`, and `_resetDefaultsYamlCache()` for the rare test that mutates. Refactored `mmt-defaults-yaml-task79.test.ts`, `mmt-defaults-yaml-task82.test.ts`, and `permission-model-fixtures.test.ts` to drop their inline yaml-parse blocks (eliminates ~10ms/run + copy-paste fallback boilerplate). Test linkage (`tests/schemas/load-defaults-helper.test.ts`): 12 new tests covering API existence + caching/singleton behavior; 65 across all three refactored files = 77 pass total with no regressions.
...
**Task 92 Completion Notes:** Done. Closes Phase 11.2. All 5 version fields bumped: synthex 0.5.1 → 0.5.2, synthex-plus 0.2.0 → 0.2.1, marketplace.json top-level + both plugin entries match. New CHANGELOG entry `[synthex 0.5.2 / synthex-plus 0.2.1] - 2026-04-29` covers the seven Phase 11.2 task surfaces (probe-caching, requestApproval id-correlation, sandbox_violation detection, Safe-name assertion, non-TTY guard, Known Limitations, loadDefaultsYaml helper) and the new sandbox.sb default profile. Test linkage (`tests/schemas/release-task92.test.ts`): 17 new tests across 5 [T] criteria — synthex version (3 sub-tests), synthex-plus version (3), marketplace match incl. cross-version sync (5), CHANGELOG entry incl. all-task-surface keyword check (5), JSON-parse regression covered inline.
