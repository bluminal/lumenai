...
**Cost advisory verbatim copy** per D16/NFR-MMT2: `"Heads up: this pool will keep {reviewer_count} reviewers idle for up to {ttl_minutes} minutes. Estimated idle cost: ~{cost_estimate}. Continue?"` (interpolated; embed verbatim in command body and in Task 46(e) fixture). | L | Phase 3; parent Phase 3 (multi-model preflight) | in progress (iter 5) |
