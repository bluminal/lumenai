...
| 45 | Author Layer 1 schema validators: (a) `tests/schemas/start-review-team-output.ts` — confirmation output shape, cost-warning conditional per D16, `submission_timeout_seconds` row; (b) `tests/schemas/list-teams-output.ts` — table column structure, State enum (all four including `stopping`), TTL integer-ness; (c) `tests/schemas/stop-review-team-output.ts` — per-pool status messages, pre-prompt table display (no-args path). Vitest with normal and edge-path samples. | M | Tasks 41, 42, 43 | in progress (iter 5) |
