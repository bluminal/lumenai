...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 16 | FR-HM6 across the 12 specialists: Interaction tables → `docs/agent-interactions.md`; Future Considerations → `docs/roadmap.md`; Scope Boundaries → 2 lines (keep "Overlap"); delete the tech-lead/lead-frontend registries; the design-system registry becomes a pointer; keep terraform routing; drop orchestrator bookkeeping | M | None | done |
...
| 18 | FR-HM13 + FR-HM42: other-hosts line after every `${CLAUDE_PLUGIN_ROOT}/scripts` call; depth-1 inline rule in `tech-lead.md`, the orchestrator, and `next-priority.md` | S | Task 17 | done |
**Task 16 Acceptance Criteria:** `[T]` `agent-boilerplate.test.ts`: each specialist is ≥ 1.5 KB smaller than the recorded size; H1s unchanged; `## Output Format` byte-identical. `[T]` Pass: standing-pool-cleanup headings, `orchestrator-md`, 7 `*-adapter-md`, `context-bundle-assembler-md`, `audit-writer-md`, `--check`. `[T]` Layer 2: no verdict change on 3 code-reviewer and 7 security-reviewer fixtures. → done in `1e60dcb`: all 12 specialists 1,541–1,959 bytes smaller (146.9 KB → 126.9 KB); tech-lead and lead-frontend registries deleted; orchestrator trimmed 2.1 KB with locked strings intact; Interaction tables → `docs/agent-interactions.md`, Future Considerations → new `docs/roadmap.md`. `agent-boilerplate.test.ts` (126 cases: size floor, H1 identity, Output Format byte-identity, no leftover headings, doc subsections). Layer 2: 8/10 fixtures stable before/after; the 2 `clean-code` fixtures vary equally on the original prompt (sampling, not regression); table in `docs/testing.md`. **Defect found:** `tests/helpers/invoke-agent.ts` and `claude-provider.js` pass the agent file PATH as literal `--system-prompt` text, so existing Layer 2 runs never loaded agent prompts; folded into Task 27 (eval baseline) as a prerequisite fix.
...
**Task 18 Acceptance Criteria:** `[T]` `portability-prose`: every plugin-root script line has an other-hosts line; the depth-1 sentence is in all 3 files. `[T]` `next-priority-auto-decide.test.ts:84-100` passes. → done in `c4dbf70`: canonical other-hosts sentence (names Codex, Gemini CLI, OpenCode, Grok, Hermes and the `plugin_root` fallback) at both script-call sites (next-priority idle wait, loop.md Step 7); depth-1 inline rule byte-identical in tech-lead, orchestrator, next-priority and nowhere else. `portability-prose.test.ts` Task 18 blocks (35 tests); next-priority-auto-decide, native-looping-*, loop-command all pass unmodified.

**Parallelizable:** {16, 17} → 18 (18 and 17 share `next-priority.md`).
**Milestone Value:** 400–500 fewer tokens per specialist spawn; inline delegation on depth-1 hosts. **Status: complete (2026-09-25)** — specialists 146.9 → 126.9 KB (12 agents), emphasis scaffolding removed, portability sentences in place; suite 153 files / 4499 passed / 8 skipped. `[O]` per-spawn delta and the Layer 2 verdict table recorded in `docs/testing.md` (Task 16).
...
| 22 | FR-HM9: codex/opencode activation captures the catalog block at loopback and asserts the `harnesses.mjs` budget | M | Task 21 | done |
| 23 | Canary tool behavior (NFR-HM4, FR-HM7, FR-HM12; D22): Codex/OpenCode `*-canary.mjs` gain (a) a `Workflow`-step probe (≤ 1 attempt, no retry) and (b) a no-injected-context probe (the instruction file is Read). Tasks 46 and 49 add cases. | M | Tasks 20, 21 | done |
**Task 19 Acceptance Criteria:** `[T]` `wrapper-catalog.test.ts`: each description ≤ 120, total < 6,000, agent wrappers carry `user-invocable: false` + `openai.yaml`. `[T]` `contract.mjs:46-74` accepts the `agents/` subdir; `codex-plugin.test.ts:38-45` and `grok-plugin.test.ts:45-54,63-70` pass. `[T]` The baseline capture is saved under `tests/compat/baselines/`. `[T]` `wrapper-catalog.test.ts`: every command wrapper description equals its `COMMAND_DESCRIPTIONS` table entry, and no file under `plugins/synthex/commands/` has a frontmatter `description:` key (guards the OQ-2 Codex duplicate-skill migration, which is silent and only gated by a 4,000-byte cap that the prompt diet will push commands under). `[T]` The `tests/compat/baselines/` Codex capture asserts the `skills/list` synthex:* count equals the generated wrapper count and contains no `source-command-` name. → done in `3ba0827`: generator-side `COMMAND_DESCRIPTIONS`/`AGENT_DESCRIPTIONS` tables (≤ 120 chars each, total < 6,000); command wrappers gain `argument-hint`, `compatibility`, `metadata.short-description`; agent wrappers gain `user-invocable: false` and `agents/openai.yaml` (`allow_implicit_invocation: false`, schema verified against the Codex docs). Baselines under `tests/compat/baselines/` with reusable capture scripts. Codex catalog 7,476 → 4,156 chars (0 blank); OpenCode `<available_skills>` 16,037 → 12,717 bytes (79%; Task 22 owns the ≤ 60% target). Codex and OpenCode offline + activation 46/46. `wrapper-catalog.test.ts` (9 tests). Decision: `compatibility`/`short-description` on command wrappers only.
**Task 20 Acceptance Criteria:** `[T]` `wrapper-catalog`: the table matches the constant byte-for-byte and both rules are present in every wrapper. `[T]` The `grok-plugin.test.ts:45-54` half-length check holds (escalate to the PM before relaxing it); `--check` passes. → done in `ebd93f7`: FR-HM12 table rendered from `host-matrix.mjs` into a generated `docs/tool-map.md` (single source) that every wrapper step 4 references; the two rules (skip-once, adopt-inline) and the Hermes `read_file` footnote are inline in all 46 wrappers; intro sentence and generator header name Hermes; README gains the Hermes install recipe. Deviation (pre-authorized fallback): the table is not embedded per wrapper because that tripped an `opencode debug skill` stdout truncation at ~146 KB. `wrapper-catalog.test.ts` "wrapper tool-name map" block; grok half-length check passes unmodified; Codex and OpenCode offline + activation 46/46.
...
**Task 22 Acceptance Criteria:** `[T]` Activation fails over budget: OpenCode `<available_skills>` block ≤ measured + 5% (13,353 bytes; the original ≤ 60% target was withdrawn because 8,044 of the 12,717 bytes are host-rendered markup and absolute paths, and 60% would need ~23-char descriptions) and description bytes ≤ 6,000; Codex has 0 blank descriptions and ≤ 6,000 rendered chars. → done in `46e6ade`: `catalog` NDJSON phase in codex/opencode activation, budgets in `harnesses.mjs` (`catalogBudgets`), over-budget failure proven by lowering the budget to 10; measured Codex 4,156 chars / 0 blank, OpenCode 12,717 bytes; `catalog-budget.test.ts` (10 tests). Amendment A17 (PRD §8) approved by A.J. Brown on 2026-09-26.
**Task 23 Acceptance Criteria:** `[T]` The `cross-harness-compat.test.ts:133-160` canary-shape checks cover the new probes. → done in `ad37968`: `tests/compat/lib/tool-probes.mjs` (Workflow-step skip-once probe and no-injected-context Read probe with per-host attempt definitions), wired into codex/opencode canary scenarios; `compat-canary-tool-probes.test.ts` (18 tests over 8 authored fixtures); canary-shape block extended. Live canary pending the first credential-gated run on main; detector shapes are best-effort until then.

**Parallelizable:** 19 → 20 → 21 → {22, 23} (shared generator); Milestone 2.4 runs alongside.
**Milestone Value:** Smaller catalogs everywhere and no Claude Code double load. **Status: complete (2026-09-26)** — Codex catalog 7,476 → 4,156 chars (0 blank), OpenCode block 16,037 → 12,717 bytes, no Claude Code double load, budgets enforced in activation, canary tool probes wired. Phase 2 complete.
...
| 26 | FR-HM44: `code-reviewer` Step 2 becomes an inline scan gated on `code_review.spec_inline_bytes` (64 KB); orchestrator Stage 8g replaces `Date.now()` with a counter rotation | S | None | done |
...
**Task 26 Acceptance Criteria:** `[T]` Step 2 has no "spawn a sub-agent" and names the key; `orchestrator-md` asserts no `Date.now`; the default is present. → done in `0a57d6e`: code-reviewer Step 2 is an inline size-gated scan (`code_review.spec_inline_bytes: 65536` in defaults.yaml and CLAUDE.md); Sub-Agent Registry table removed (13,790 → 12,105 bytes); orchestrator Step 8g rotates by an invocation counter in `.synthex/state.json` instead of `Date.now()`. `code-reviewer-md.test.ts` (new), `orchestrator-md.test.ts` and `load-defaults-helper.test.ts` Task 26 blocks.

**Parallelizable:** 24–26 concurrently.
**Milestone Value:** Gemini multi-model review works; orchestrator config references resolve. **Status: complete (2026-09-26)** — missing config keys added, Gemini adapter invocation fixed, code-reviewer dead step and orchestrator clock seed removed.
...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 27 | FR-HM34: `plugins/synthex/evals/` with 18 planted-issue cases, deterministic graders, ablation on, a hash-keyed skip wrapper, and a manual `agent-tests.yml` job (`--threshold 0.67`); baseline over ≥ 3 runs (D28) **Prerequisite (Task 16 finding):** fix `tests/helpers/invoke-agent.ts` and `claude-provider.js`, which pass the agent file path as literal `--system-prompt` text; read the file and pass its content (or `--append-system-prompt` with content) so Layer 2 actually loads agent prompts; also wire `code-reviewer` cases into `promptfoo.config.yaml` (it has none today). | M | Task 3 | pending |
