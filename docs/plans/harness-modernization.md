...
| Q7 | (OQ-1 follow-up) Does an `opencode.json` `skills.paths: ["portable-skills"]` entry restore OpenCode discovery of the renamed wrapper tree? Task 5 showed OpenCode discovery is hardcoded to `.claude/skills/**` and `.agents/skills/**` (0/46 under a plain rename). | Task 21 OpenCode sub-item; otherwise the OpenCode gap is accepted per Q6 and recorded in the install recipe. | Open (Task 21) |
...
| 8 | Spike OQ-7 (FR-HM45) on the current tree: does `hermes skills list` see `.agents/skills/`, can `skill_view` read `../../commands/`, and does Skills Guard flag anything? **Fallback:** `hermes -z` + `skills_list()` inventory, with install-time path rewrite in the recipe (no wrapper variant). | S | None | in progress |
