...
| 20 | FR-HM12: emit the tool-map table and the two rules (skip-once, adopt-inline) from `host-matrix.mjs` into every wrapper; the generator header lists all harnesses, including Hermes (FR-HM45) **Hermes recipe (Task 8):** copy the whole plugin tree to `<project>/.agents/synthex/`, symlink `.agents/skills` → `.agents/synthex/portable-skills`, never nest `commands/`/`agents/` inside a skill folder (Skills Guard quarantines them), then `hermes skills trust` once per checkout; per-skill `hermes skills install` has no local-path form. | M | Tasks 8, 19 | in progress |
...
| # | Task | Complexity | Dependencies | Status |
|---|------|-----------|--------------|--------|
| 24 | FR-HM44: `multi_model_review.context.{max_bundle_bytes,max_file_bytes,convention_paths,spec_paths}` and `per_reviewer_timeout_seconds` in `defaults.yaml` | S | None | in progress |
